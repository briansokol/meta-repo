...
> Only present in meta-repo projects where `repos/` contains multiple cloned repositories.
> Omit this section for single-repo projects.

List each repository this feature will modify. Repository names match the directories in `repos/`.

| Repository | Role in this Feature | Change Type |
|------------|----------------------|-------------|
| `todo-contracts` | OpenAPI spec — source of truth | New endpoints |
| `todo-backend` | FastAPI backend | New model, migration, router, service |
| `todo-frontend` | React SPA | New components and hooks |
| `todo-infra` | Docker Compose | No changes expected |

**Change Types**: New file(s) / Modify existing / No changes expected / TBD

> Repo selection is confirmed by the user during design generation and stored in `spec.json.affected_repos`.
> `sdd-impl` reads `spec.json.affected_repos` to set up git worktrees before implementation begins.
> The change order should follow the Cross-Repo Change Pattern in `structure.md` steering (contracts → backend → frontend → infra).
