...
**Set up git worktrees** (when `repos/` directory exists and is non-empty):

Read `spec.json.affected_repos`. If the field is empty or missing, check if `repos/` is non-empty and warn:
"`affected_repos` is empty — worktrees will not be set up. Run `/sdd-spec-design {feature}` to populate affected repos, or proceed without worktrees."
...
If `repos/` does not exist or `affected_repos` is empty after the warning, skip this block.
...
- Construct a prompt by combining the template with task-specific context:
  ...
  - **Worktree paths** (when worktrees were set up): Include `WORKTREE_PATHS` map and instruct: "Make all code changes inside the worktree paths below, NOT in `repos/`. Each worktree is already on the correct feature branch." Example: `specs/{feature}/repos/todo-backend/` for backend changes.
...
- The reviewer must apply the `sdd-review` protocol to this task-local review.
...
- **APPROVED** → before marking the task `[x]` or making any success claim, apply `sdd-verify-completion` using fresh evidence from the current code state; then mark task `[x]` in tasks.md and perform selective git commit
...
- **Multi-repo worktrees**: If the task changed files in one or more worktrees, commit each worktree separately:
  `git -C specs/{feature}/repos/{repo-name}/ add <explicit-files>`
  `git -C specs/{feature}/repos/{repo-name}/ commit -m "feat({feature-name}): {task description}"`
- NEVER commit meta-repo management files (`tasks.md`, `spec.json`) inside a worktree — those belong to the meta-repo commit only.
- If no worktrees exist, commit in meta-repo as usual (existing behavior).
...
- The debugger must apply the `sdd-debug` protocol to this failure investigation.
...
**`(P)` markers**: Tasks marked `(P)` in tasks.md indicate they have no inter-dependencies and could theoretically run in parallel. However, sdd-impl processes them sequentially (one at a time) to avoid git conflicts and simplify review. The `(P)` marker is informational for task planning, not an execution directive.
...
- **REVIEW**: Apply `sdd-review` before marking the task complete. If the host supports fresh subagents in manual mode, use a fresh reviewer; otherwise perform the review in the main context using the `sdd-review` protocol. Do NOT continue until the verdict is parseably `APPROVED`.
- **MARK COMPLETE**: Only after review returns `APPROVED`, apply `sdd-verify-completion`, then update the checkbox from `- [ ]` to `- [x]` in tasks.md.
...
- If `git worktree add` fails (e.g., branch already checked out elsewhere), report the exact error and stop
- Do NOT proceed with implementation in the wrong directory
- Suggested Action: "Run `git -C repos/{repo-name} worktree list` to inspect existing worktrees. Ensure no other worktree is using `feat/{feature-name}` in this repo."
