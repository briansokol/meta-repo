...
- **Worktree paths** (when repos/ exists): A map of `WORKTREE_PATHS: { repo-name → path }` specifying where to make code changes. Make all code changes inside the provided worktree paths, NOT directly in `repos/`. Each worktree is already on the correct feature branch. If WORKTREE_PATHS is absent, work in the standard `repos/{repo-name}/` directories.
...
- If WORKTREE_PATHS is provided, resolve all file paths relative to the provided worktree path for the relevant repo. For example, `app/models/tag.py` in `todo-backend` means editing `specs/{feature}/repos/todo-backend/app/models/tag.py`.
...
- When WORKTREE_PATHS is provided, make ALL file changes inside worktree paths. Never edit files in `repos/` directly when worktrees are active.
