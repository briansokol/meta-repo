...
Per-task checks are the reviewer's responsibility during `/sdd-impl`. This skill does **not** re-check:
...
- Scan `specs/` for features with completed tasks `[x]`
...
- Read `specs/<feature>/tasks.md` for task list and Implementation Notes
...
**Multi-repo validation** (when `spec.json.affected_repos` is non-empty):
- For each affected repo, discover its validation commands separately:
  - Check if a worktree exists at `specs/{feature}/repos/{repo-name}/`. If it does, derive commands from the worktree. If no worktree exists, fall back to `repos/{repo-name}/`.
  - Read each repo's CLAUDE.md for the canonical test/dev command (e.g., `pytest tests/ -v` for todo-backend, `npm run dev` for todo-frontend).
  - Read project manifests (`package.json`, `Makefile`, `pyproject.toml`) in the resolved repo path for canonical commands.
- Build a per-repo validation map: `{ repo-name → { TEST_COMMANDS, BUILD_COMMANDS, SMOKE_COMMANDS, working_dir } }`
...
For each affected repo (when `spec.json.affected_repos` is non-empty):
- Resolve the working directory: worktree path (`specs/{feature}/repos/{repo-name}/`) if it exists, otherwise `repos/{repo-name}/`
- Run the per-repo canonical test command (from the validation map built in Step 2) inside the resolved working directory
- Report per-repo pass/fail with exit code
- If any repo's tests fail → NO-GO

For single-repo projects: run the discovered canonical full-test command. Use the exit code.
If the canonical test command cannot be identified for any repo → `MANUAL_VERIFY_REQUIRED`
...
For each affected repo that has a meaningful runtime (e.g., todo-backend, todo-frontend — not todo-contracts or todo-infra):
- Resolve the working directory: worktree path if it exists, otherwise `repos/{repo-name}/`
- Run the per-repo canonical smoke command inside the resolved working directory
- Report per-repo pass/fail
- If any repo's smoke check produces a runtime crash, unhandled exception, module-load failure, native ABI mismatch, or missing required env/config → NO-GO

For single-repo projects: run the discovered canonical smoke command as above.
...
- MECHANICAL_RESULTS:
  - Tests: (per repo when multi-repo, otherwise single result)
    - todo-contracts: PASS | FAIL | N/A (command and exit code)
    - todo-backend: PASS | FAIL (command and exit code)
    - todo-frontend: PASS | FAIL (command and exit code)
  - TBD/TODO grep: CLEAN | <count> matches
  - Secrets grep: CLEAN | <count> matches
  - Smoke boot: (per repo when multi-repo, otherwise single result)
    - todo-backend: PASS | FAIL | MANUAL_REQUIRED
    - todo-frontend: PASS | FAIL | MANUAL_REQUIRED
- INTEGRATION:
...
- **`repos/` absent or `affected_repos` empty**: Run validation against `repos/` directories directly (existing behavior). No warning needed — single-repo is the silent default.
- **Worktrees absent despite `affected_repos` being non-empty**: Fall back to running validation in `repos/{repo-name}/` for each affected repo. Warn: "Worktrees not found at `specs/{feature}/repos/`. Running validation in `repos/` instead. Run `/sdd-impl {feature}` to set up worktrees."
