...
- **Affected repos** (when `repos/` exists): Read `spec.json.affected_repos` to understand which repos this feature touches. This informs `_Boundary:_` qualifier prefixes in Step 2.
...
- **Multi-repo boundary qualifiers** (when `spec.json.affected_repos` is non-empty): Prefix `_Boundary:_` annotations with the repo name when components exist in multiple repos or when the boundary would otherwise be ambiguous. Format: `_Boundary: todo-backend/TagService_`. Single-repo tasks or tasks where the repo is obvious from context may omit the prefix.
...
- **`repos/` absent or `affected_repos` empty**: Use standard unqualified `_Boundary:_` annotations (existing behavior). No warning needed.
