...
- Read `specs/$ARGUMENTS/spec.json` for metadata and phase status
- Read `specs/$ARGUMENTS/brief.md` if it exists
- Read existing files: `requirements.md`, `design.md`, `tasks.md` (if they exist)
- Check `specs/$ARGUMENTS/` directory for available files
- Read `.sdd/steering/roadmap.md` if it exists and this spec appears in it
...
**Spec Not Found**:
- **Message**: "No spec found for `$ARGUMENTS`. Check available specs in `specs/`"
