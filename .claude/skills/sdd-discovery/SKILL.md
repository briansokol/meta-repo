...
- **Specs inventory**: Glob `specs/*/spec.json`, read each spec.json for `name`, `phase` fields and `approvals` status. Note feature names and their current status.
...
- **Available repos** (when `repos/` directory exists): List the contents of `repos/` to identify available repositories. Store as `available_repos` list for use in Step 4. Do NOT read into the repos at this stage.
...
- **Multi-repo**: When `available_repos` is non-empty, instruct the subagent to also explore each `repos/{repo-name}/` directory and return a per-repo summary (existing implementations, stubs, patterns). This ensures the codebase exploration covers all repos, not just the meta-repo root.
...
3. **Boundary candidates**: What are the natural responsibility seams in this work? Where could this be split so implementation can proceed independently?
   - **Multi-repo context** (when `available_repos` is non-empty): Which repositories will this feature touch? (Detected repos: {available_repos}). This informs which CLAUDE.md files to read and helps the design phase propose the initial `affected_repos` list.
...
Use the Write tool to create `specs/<feature-name>/brief.md` with this structure:
...
- `specs/<feature>/brief.md` for every feature listed under `## Specs (dependency order)`
...
Then create `specs/<feature>/brief.md` for **every** feature listed under `## Specs (dependency order)` using the Path C brief format. This enables parallel spec creation via `/sdd-spec-batch`.
...
**`repos/` absent or empty**:
- Skip multi-repo detection in Step 1 (`available_repos` is empty)
- Omit repo-related context from Step 4, question 3
- No warning needed — single-repo is the silent default
