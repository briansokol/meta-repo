...
- Read `specs/{feature}/brief.md` if it exists (discovery context: problem, approach, scope decisions, boundary candidates)
...
- **Multi-repo context** (when `repos/` directory exists and is non-empty):
  - List the repos present in `repos/`
  - For each present repo, read `repos/{repo-name}/CLAUDE.md` to understand its stack, conventions, and known stubs relevant to this feature area
  - Provide these findings to the "Codebase hints" subagent in Step 2 so it explores all repos, not just the meta-repo root
  - Goal: inform requirement scope with existing state; do NOT select or commit to repos at this phase
...
- Write `specs/{feature}/requirements.md` only after the requirements review gate passes
...
- **`repos/` directory absent**: Skip all multi-repo context steps; proceed as a single-repo project. No warning needed — single-repo is the silent default.
