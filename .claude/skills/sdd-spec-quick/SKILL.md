...
1. **Check for Brief**:
   - If `specs/{feature-name}/brief.md` exists (created by `/sdd-discovery`), read it for discovery context (problem, approach, scope, constraints)
...
4. **Create Directory**:
   - Use Bash: `mkdir -p specs/{feature-name}` (skip if already exists from discovery)
...
6. **Output Progress**: "Phase 1/4 complete: Spec initialized at specs/{feature-name}/"
...
Invoke `/sdd-spec-requirements {feature-name}` via the Skill tool.
...
Invoke `/sdd-spec-design {feature-name} -y` via the Skill tool. The `-y` flag auto-approves requirements.
...
Invoke `/sdd-spec-tasks {feature-name} -y` via the Skill tool.
...
Skipped: /sdd-validate-gap, /sdd-validate-design

Sanity review: PASSED | FOLLOW-UP REQUIRED

Next Steps:
1. Review generated specs (especially design.md)
2. Optional: `/sdd-validate-gap {feature}`, `/sdd-validate-design {feature}`
...
- Suggest targeted follow-up with `/sdd-spec-design {feature}`, `/sdd-spec-tasks {feature}`, or manual edits depending on the finding
